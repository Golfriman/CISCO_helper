 ...
 * IP_ADDRESS_DEVICES - ip интрефейса к которому идет подключение
  ...
  * ipv6 unicast-routing
  ...
  2. Работаем справой частью. 
  3. Разрезаем мак адрес пополам: [00E0.B0] [72.39B4]
  4. Вставляем FF:FE между двумя частями получаем 00E0:B0FF:FE72:39B4
  5. 7 бит начиная слева необходимо инвертировать то есть получаем: 02E0:B0FF:FE72:39B4
  ```
  Example
  MAC: AAAA.BBBB.CCCC
  AAAA:BBFF:FEBB:CCCC
  ...
  A8AA:BBFF:FEBB:CCCC
  ```
  6. Остальные 64 бита IPv6 адреса необходимо брать из варианта
  * В итоге получается ХХХХ:XXXX:XXXX:XXXX:00E0:B0FF:FE72:39B4
