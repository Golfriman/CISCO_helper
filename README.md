...
<h2 name=13>Настройка Port-Security</h2>
  ...
  <ol type="1">
   ...
   <li> shutdown - В режиме shutdown (отключение) порт, на котором было обнаружено нарушение, полностью отключается. Это означает, что порт становится неактивным и все трафик на нем останавливается. Для возобновления работы порта администратор должен вручную включить его.
 </ol>
 </ul>
